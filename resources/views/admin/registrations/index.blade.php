...
@section('title', 'Pendaftar Lomba')
@section('heading', 'Pendaftar Lomba')
    ...
    <table class="w-full text-left">
        ...
        @forelse ($registrations as $registration)
            <tr class="align-top">
                ...
                <td class="py-4 pr-4">{{ $registration->institution ?: '-' }}</td>
                <td class="py-4 pr-4 max-w-xs">{{ $registration->address ?: '-' }}</td>
                <td class="py-4 pr-4 min-w-56">
                    <form method="POST" action="{{ route('admin.registrations.status', $registration) }}" class="space-y-1" data-status-form>
                        @csrf
                        @method('PATCH')
                        <select name="status" data-current-status="{{ $registration->status }}" class="w-44 rounded-xl border-surface-variant bg-white/80 text-sm font-semibold">
                            @foreach ($statuses as $status => $label)
                ...
                <td class="py-4 whitespace-nowrap">{{ $registration->submitted_at?->format('d/m/Y H:i') }}</td>
            </tr>
        @empty
            <tr>
